...
const WpPost = ({data}) => {
	...
	if(data.wp.themeGeneralSettings.stateBreedOptions.statesBackgroundImage) {
		bg = `		
		<style>
			.post-archive::before {
				background-image: url("${data.wp.themeGeneralSettings.stateBreedOptions.statesBackgroundImage.sourceUrl}") !important;
			}
		</style>`
	}
	...
	if(data.wp.themeGeneralSettings.stateBreedOptions.statesBackgroundImageMobile) {
		bgMob = `		
		<style>
			@media only screen and (max-width: 600px) {
				.post-archive::before {
					background-image: url("${data.wp.themeGeneralSettings.stateBreedOptions.statesBackgroundImageMobile.sourceUrl}") !important;
				}
			}
		</style>`
	}
			...
			<div class="post-archive" id="main-content">
				...
				<div class="container container--no-flex">
					<div class="post-archive__text">
						<h1>Find the best pet insurance in your State</h1>
						<p>Whether you've got a Labrador in Louisiana or a Terrier in Texas, find out all you need to know about pet insurance where you live</p>
					</div>
						...
						<div class="post-archive__overall-select-wrap">
							<div class="post-archive__select-wrap">
								<select name="state" id="state">
									<option value="javascript:void(0);" disabled selected>California</option>
									{
										(data.allWpState.nodes).map((state, index) => {
											return (
												<option value={state.uri}>
													{
														(state.states.archiveTitle) ? state.states.archiveTitle : state.title
													}
												</option>
											)
										})
										...
										(data.allWpState.nodes).map((state, index) => {
											return (
												<div class="post-archive__search-modal-item">
													<a href="javascript:void(0);" data-val={state.uri}>
														{
															(state.states.archiveTitle) ? state.states.archiveTitle : state.title
														}
													</a>
					...
					<div class="post-archive__breadcrumbs">
						<p class="breadcrumbs"><span><span><a href="https://www.petted.com">Home</a> &gt; <span><a href="https://www.petted.com">Pet Insurance</a> &gt; <span class="breadcrumb_last" aria-current="page">By State</span></span></span></span></p>
					</div>
				</div>
			</div>

			<StateMap page='/states/' />
